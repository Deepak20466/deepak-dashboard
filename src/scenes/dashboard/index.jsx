...
const Dashboard = () => {
  ...
  const [combinedTransactions, setCombinedTransactions] = useState(mockTransactions);
  const [totalTeam, setTotalTeam] = useState(0);

  useEffect(() => {
    // 1. Pull Team Data
    const savedTeam = JSON.parse(localStorage.getItem("teamData") || "[]");
    // 2. Pull Contacts Data
    const savedContacts = JSON.parse(localStorage.getItem("contactsData") || "[]");

    // 3. Transform Team Members into "Activity" format
    const teamActivity = savedTeam.map((user) => ({
      txId: `TEAM-${user.id.toString().slice(-4)}`,
      user: user.name,
      date: "Recent",
      cost: "MEMBER",
    }));

    // 4. Transform Contacts into "Activity" format
    const contactActivity = savedContacts.map((contact) => ({
      txId: `CONT-${contact.id.toString().slice(-4)}`,
      user: contact.name,
      date: "Contact",
      cost: "SYNCED",
    }));

    // Combine everything: Newest Team + Newest Contacts + Original Mock Transactions
    setCombinedTransactions([...teamActivity, ...contactActivity, ...mockTransactions]);
    setTotalTeam(savedTeam.length);
  }, []);

  return (
    // Style Fix: m="0 20px" removes top margin to move it up
    <Box m="0 20px">
      {/* HEADER */}
      ...
      >
        ...
        <Box gridColumn="span 3" backgroundColor={colors.primary[400]} display="flex" alignItems="center" justifyContent="center">
          <StatBox title={totalTeam.toString()} subtitle="New Team Members" progress="0.30" increase="+5%" icon={<PersonAddIcon sx={{ color: colors.greenAccent[600], fontSize: "26px" }} />} />
        </Box>
        <Box gridColumn="span 3" backgroundColor={colors.primary[400]} display="flex" alignItems="center" justifyContent="center">
        ...
        <Box gridColumn="span 4" gridRow="span 2" backgroundColor={colors.primary[400]} overflow="auto">
          <Box display="flex" justifyContent="space-between" alignItems="center" borderBottom={`4px solid ${colors.primary[500]}`} p="15px">
            <Typography color={colors.grey[100]} variant="h5" fontWeight="600">Recent Activity</Typography>
          </Box>
          {combinedTransactions.map((transaction, i) => (
            <Box key={`${transaction.txId}-${i}`} display="flex" justifyContent="space-between" alignItems="center" borderBottom={`4px solid ${colors.primary[500]}`} p="15px">
              ...
              {/* DATE BOX: Fixed width to keep alignment straight */}
              <Box color={colors.grey[100]} sx={{ width: "80px", textAlign: "left" }}>
                {transaction.date}
              </Box>

              {/* COST BOX: Fixed width for buttons/labels to keep alignment straight */}
              <Box 
                sx={{
                  backgroundColor: 
                    transaction.cost === "MEMBER" ? colors.blueAccent[500] : 
                    transaction.cost === "SYNCED" ? colors.greenAccent[600] : 
